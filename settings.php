...
if ($ADMIN->fulltree) {
    ...
    $setting = new admin_setting_configtextarea('planner/upcomingemail',
        $name,
        $description,
        $default);
    ...
    $setting = new admin_setting_configtextarea('planner/dueemail',
        $name,
        $description,
        $default);
    $setting->set_force_ltr(false);
    ...
    $plannersettings->add(new admin_setting_configtext('planner/frequencyemail',
        get_string('frequencyemail', 'mod_planner'), get_string('frequencyemailhelp', 'mod_planner'), 2, PARAM_INT, 2));
    ...
    $setting = new admin_setting_configtextarea('planner/deletedactivityemail',
        $name,
        $description,
        $default);
    $setting->set_force_ltr(false);
    ...
    $setting = new admin_setting_configtextarea('planner/deletedactivitystudentemail',
        $name,
        $description,
        $default);
    $setting->set_force_ltr(false);
    ...
    $setting = new admin_setting_configtextarea('planner/changedateemailtemplate',
        $name,
        $description,
        $default);
    $setting->set_force_ltr(false);
    ...
    for ($i = 1; $i <= 6; $i++) {
        $plannersettings->add(new admin_setting_configtext('planner/step'.$i.'name',
            get_string('stepnamesettings', 'mod_planner', $i), '', get_string('step'.$i.'default', 'mod_planner'), PARAM_RAW, 50));
        $plannersettings->add(new admin_setting_configtext('planner/step'.$i.'timeallocation',
            get_string('steptimeallocationsettings', 'mod_planner', $i), get_string('enternumber', 'planner'),
            get_string('step'.$i.'defaultallocatin', 'mod_planner'), PARAM_INT, 3));
...
if (has_capability('mod/planner:managetemplates', context_system::instance())) {
        $ADMIN->add('modsettings', new admin_category('modplanner',
            get_string('modulename', 'planner'), $module->is_enabled() === false));
        $ADMIN->add('modplanner', $plannersettings);
        $ADMIN->add('modplanner',
