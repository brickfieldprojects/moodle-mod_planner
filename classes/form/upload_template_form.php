...
class upload_template_form extends \moodleform {
     ...
     * @return array
     */
    private static function validate_template_structure(array $json): array {
        $errs = [];
        $keys = [];
        ...
        foreach ($required as $key => $type) {
            ...
            if ($type === 'array' && !is_array($json[$key])) {
                $errs[] = get_string('invalidkeytype', 'mod_planner', $key);
            }
        }

        if (!empty($keys)) {
            $keystr = implode(', ', $keys);
            $errs[] = get_string('missingkey', 'mod_planner', $keystr);
        }
